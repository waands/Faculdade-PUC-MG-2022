...
public class GameTree {
    ...
    */
    ...
    public static void main (String[] args) {
        ...
        ArvoreBinaria gameList = new ArvoreBinaria();    
        ...
        for (int k = 0; k < operacoes; k++) {
            try{
                ...
                objReader = new BufferedReader(new FileReader("/tmp/games.csv"));
                while ((line = objReader.readLine()) != null) {
                    String[] gameId = line.split(",", 1);
                    if (input.length == 2 && (gameId[0].equals(input[1]) ||
                                              gameId[1].compareTo(input[1]))){
...
public class ArvoreBinaria {
    ...
    private boolean pesquisar(String x, No i) {
      ...
      } else if (x.compareTo(i.elemento.getName())== 1) {
         resp = true;
         MyIO.print("raiz ");

      } else if (i.elemento.getName().compareTo(x)== 1) {
         MyIO.print("esq ");
    ...
    private No inserir(Game x, No i) throws Exception {
      ...
      } else if (i.elemento.getName().compareTo(x.getName())== 1) {
         i.esq = inserir(x, i.esq);

      } else if (x.getName().compareTo(i.elemento)== 1) {
    ...
    public void inserirPai(Game x) throws Exception {
      if(raiz == null){
         raiz = new No(x);
      } else if(raiz.elemento.getName().compareTo(x.getName()) == 1){
    ...
    private void inserirPai(Game x, No i, No pai) throws Exception {
        if (i == null) {
         if(pai.elemento.getName().compareTo(x.getName()) == 1){
            pai.esq = new No(x);
         } else {
            pai.dir = new No(x);
         }
      ...
      } else if (x.getName().compareTo(i.elemento.getName()) == 1) {
